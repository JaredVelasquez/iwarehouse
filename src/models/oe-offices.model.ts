...
@model({settings: {idInjection: false, mssql: {schema: 'dbo', table: 'oe_offices'}}})
export class OeOffices extends Entity {
  @property({
    type: 'number',
    required: false,
    precision: 10,
  ...
  @property({
    ...
    precision: 10,
    scale: 0,
    mssql: {columnName: 'officeUserId', dataType: 'int', dataLength: null, dataPrecision: 10, dataScale: 0, nullable: 'NO'},
  })
  officeUserId: number;
  ...
  constructor(data?: Partial<OeOffices>) {
    super(data);
  }
}

export interface OeOfficesRelations {

}

export type OeOfficesWithRelations = OeOffices & OeOfficesRelations;
